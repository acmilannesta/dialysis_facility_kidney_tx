...
PROC IMPORT OUT= dfc2016 
            DATAFILE= "C:\Users\zhangxyu\Desktop\JAMA_paper_revision\DFC_dwnld03152017_clean.csv" 
            DBMS=CSV REPLACE;
     GETNAMES=YES;
     DATAROW=2; 
RUN;
...
libname dfr18 "C:\Users\zhangxyu\Desktop\JAMA_paper_revision";
libname jgdfr18 "D:\DFR 2018_data2013-16"; /*JGander hardrive DFR folder*/
...
data demo_dfr1316
	 ...
	 if network in (1,2,3,4) 		then network_us_region_dfr=0 ; /*North East*/
else if network in (5,6,7,8,13,14) 	then network_us_region_dfr=1 ; /*South*/
else if network in (9, 10, 11, 12) 	then network_us_region_dfr=2 ; /*Midwest*/
else if network in (15,16,17,18) 	then network_us_region_dfr=3 ; /*West*/

run;
proc contents data=demo_dfr1316 varnum; run;
/* 6574 observations and 71 variables*/
proc freq data=demo_dfr1316; 
table network_us_region_dfr 
		network_us_region_dfr*network_dfr;
run;
...
run;
/**** 6524 observations**/
proc contents data=dfc_dfr varnum; run;
/*merging with USRDS crosswalk to determine 
	which facilities in DFC-DFR are Transplant Centers*/
data xref_prv_17b (keep=provhcfa ccn CTR_CD);
	set xref_prv_17;
ccn=provhcfa*1;
run;

proc sort data=xref_prv_17b FORCE; by ccn; run;
proc sort data=dfc_dfr; by ccn; run;
data dfwl.dfc_dfr_txc;
	merge	dfc_dfr (in=a)
			xref_prv_17b (keep=ccn CTR_CD);
	by ccn;
	if a;

if CTR_CD ne ' ' then txc=1; else txc=0;
run;
proc freq data=dfwl.dfc_dfr_txc;
table txc chain_class chain_class2
		(chain_class chain_class2)*txc;
run;
...
proc means data=dfwl.dfc_dfr_txc n min max mean std median q1 q3;
var socialwkr_dfr patients_n_dfr;
run;


/**** removed this step because revised the crosswalk merge...
		We are now merging the USRDS crosswalk with the patient-level
		file in Step 2....and will then merge it to DFC_DFR in Step 4.
		(JGander 11/10/18)					**/
/*STEP 4: 	Crosswalk to get [PROVUSRD]--> 6-digit [provhcfa]
/*JGander updated (11/3/18) using updated crosswalk from USRDS 2018 files*/

/*///////////////////////////////\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\*/
libname crsswlk "C:\Users\zhangxyu\Desktop\JAMA_paper_revision";
